...
\begin{itemize}
    ...
    \item \textbf{实例化}：网页 \texttt{binary-tree-bst} 演示查找/插入/删除的路径帧；数组、指针实现均为本规约的实例。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{实例化}：网页 \texttt{binary-tree-avl} 以 $bf$ 标注演示四种旋转与删除回溯；伪代码中的数组下标、节点结构体为语言实例。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{实例化}：网页 \texttt{binary-tree-rb} 以红/黑 tone 演示插入变色与旋转；哈希链地址重构为红黑树（见线性结构章）即本结构的一次复用。
\end{itemize}

\section{B 树与 B+ 树 (B-Tree / B+ Tree)}
...
\begin{itemize}
    ...
    \item \textbf{实例化}：网页 \texttt{btree}、\texttt{btree-plus} 演示 $m$ 阶分裂/合并与叶链；磁盘页、数据库索引为其工程实例，数学性质不变。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{实例化}：网页 \texttt{binary-tree-heap} 演示上滤/下滤；数组 $A$、\texttt{Swap} 为语言实例，数学本质为完全二叉树上的堆序谓词维护。
\end{itemize}
