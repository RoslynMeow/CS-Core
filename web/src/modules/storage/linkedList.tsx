...
type Cfg = { elemType: ElemType; endian: 'little' | 'big'; valuesStr: string; prevValuesStr?: string; scattered: boolean; ptrSize: 4 | 8; op: Op; pos: number; insVal: number; execTick: number; };
...
function buildNodesWithHeap(cfg: Cfg, values: number[]): { nodes: NodeInfo[]; heap: Heap; heapBase: number } {
  ...
  for (let i = 0; i < values.length; i++) {
    if (cfg.scattered && i === 2) heap.allocate(`__gap_${i}__`, 32);
    else if (cfg.scattered && i > 0) heap.allocate(`__pad_${i}__`, 6);
    const addr = heap.allocate(`node${i}`, elemSize + cfg.ptrSize) ?? heap.base;
    addrs.push(addr);
  }
  for (let i = 0; i < values.length; i++) {
    ...
    nodes.push({ idx: i, addr: addrs[i], data: v, next, size: elemSize + cfg.ptrSize, hex: hexFromBytes(bytes), bytes });
  }
...
function buildDump(cfg: Cfg) {
  ...
  // base 必须与节点真实所在的堆顶一致，否则 HEX 按 addr-base 偏移错乱
...
function gen(cfg: Cfg): Frame<Scene>[] {
  ...
  if (cfg.execTick === 0 || cfg.op === 'idle') {
    ...
    return [
      { line: 0, caption: T(empty ? '空链：$head=null$，点“示例”构造或选插入后执行' : `待执行：$L=[${vals.join(',')}]$ ${vals.length} 节点`, empty ? 'empty' : `Pending`), scene: idle },
    ];
  }
  const pos = Math.max(0, cfg.pos | 0);
  if (cfg.op === 'get') {
  ...
  if (cfg.op === 'insert') {
    ...
    return [
      ...
      { line: 1, caption: pos > 0 ? T(`链接：$L[${pos - 1}].next\\gets 0x${afterScene.nodes[pos].addr.toString(16)}$`, `link`) : T(`头插：$head\\gets 0x${afterScene.nodes[pos].addr.toString(16)}$`, `head`), scene: afterScene },
  ...
  if (cfg.op === 'delete') {
    ...
    return [
      ...
      { line: 1, caption: T(`越过：前驱 $next$ 指向后继，$free(q)$`, `unlink & free`), scene: afterScene },
      { line: 2, caption: T(`完成：$L=[${after.join(',')||'∅'}]$`, `done`), scene: afterScene },
    ];
  }
...
export const linkedListModule: ModuleDef<Scene, Cfg> = {
  ...
  Controls({ config, onChange, t, onPlay }: any) {
    ...
    const exec = () => {
      ...
      if (op === 'insert') {
        const p = draft.pos | 0;
        // 越界：拒绝执行，不改表、不持久化，交 gen 生成越界错误帧
        if (p >= 0 && p <= capVals.length) { nextStr = [...capVals.slice(0,p), Math.trunc(draft.insVal), ...capVals.slice(p)].join(','); prev = draft.valuesStr; }
      }
      else if (op === 'delete') {
        const p = draft.pos | 0;
        if (p >= 0 && p < capVals.length) { nextStr = [...capVals.slice(0,p), ...capVals.slice(p+1)].join(','); prev = draft.valuesStr; }
      }
    ...
    return (
      <div style={{ display: 'grid', gap: 8, width: '100%' }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 10px', borderRadius: 12, background: '#eef2ff', border: '1px solid #c7d2fe', flexWrap: 'wrap' }}>
          ...
          <label style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 13 }}><input type="checkbox" checked={draft.scattered} onChange={e=>set({scattered:e.target.checked})} /><span>{t(T('离散大空隙','Scattered gap'))}</span></label>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 10px', borderRadius: 12, background: '#f8fafc', border: '1px solid #e2e8f0', flexWrap: 'wrap' }}>
          ...
          {draft.op !== 'idle' && (
            <span style={{ fontSize: 11, fontFamily: 'monospace', background: invalid ? '#fef2f2' : '#f1f5f9', color: invalid ? '#dc2626' : '#64748b', border: `1px solid ${invalid ? '#fecaca' : '#e2e8f0'}`, borderRadius: 999, padding: '3px 8px' }}>
              {isZh ? `共 ${curLen} 节点` : `${curLen} nodes`}{invalid && ` · ${pos}${isZh ? ' 越界' : ' OOB'}`}
            </span>
          )}
        </div>
      </div>
    ...
    return (
      <div style={{ display: 'grid', gap: 10 }}>
        <div style={{ border: '1px solid #c7d2fe', borderRadius: 12, overflow: 'hidden', background: '#eef2ff' }}>
          <div style={{ padding: '8px 10px', fontSize: 11, fontWeight: 800, color: '#4338ca', display: 'flex', gap: 8, alignItems: 'center' }}><span>逻辑视图 · 链表</span><span style={{ fontWeight: 400, color: '#64748b' }}>head = {scene.head ? `0x${scene.head.toString(16)}` : 'null'} · {scene.nodes.length} 节点 · heapBase 0x{scene.heapBase.toString(16)}</span></div>
