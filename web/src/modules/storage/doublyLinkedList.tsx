...
type NodeInfo = { idx: number; addr: number; prev: number | null; data: number; next: number | null; size: number; hex: string; bytes: number[]; elemSize: number; ptrSize: number };
...
function gen(cfg: Cfg): Frame<Scene>[] {
  ...
  if (cfg.execTick === 0 || cfg.op === 'idle') {
    ...
    return [
      { line: 0, caption: T(empty ? '空表：$head=null$，点“示例”构造' : `待执行：$L=[${vals.join(',')}]$ ${vals.length} 节点，双向 $prev\\leftrightarrow next$`, empty ? 'empty' : `Pending`), scene: idle },
    ];
  }
  const pos = cfg.pos | 0;
  if (cfg.op === 'get') {
    ...
    for (let i = 0; i <= Math.min(pos, target.nodes.length - 1); i++) frames.push({ line: 2, caption: T(`遍历到 $L[${i}]$ @0x${target.nodes[i].addr.toString(16)}$（亦可沿 $prev$ 回退）`, `traverse to L[${i}]`), scene: buildScene(cfg, i, 'traverse') });
  ...
  if (cfg.op === 'insert') {
    ...
    const node = afterScene.nodes[pos];
    return [
      { line: 0, caption: T(`新节点 $malloc$ → $0x${node.addr.toString(16)}$`, `malloc→0x${node.addr.toString(16)}`), scene: afterScene },
      { line: 1, caption: T(`同步链接：$prev=${node.prev ? `0x${node.prev.toString(16)}` : 'null'}$，$next=${node.next ? `0x${node.next.toString(16)}` : 'null'}$`, `link prev&next`), scene: afterScene },
      { line: 2, caption: T(`完成：$L=[${after.join(',')}]$，前后驱 $prev/next$ 均已更新`, `done`), scene: afterScene },
    ];
  }
  if (cfg.op === 'delete') {
    const can = pos >= 0 && pos < origVals.length;
    ...
    const gone = buildScene(cfg, pos, 'idle').nodes[pos];
    return [
      { line: 0, caption: T(`定位 $L[${pos}]$ @0x${gone.addr.toString(16)}$，前驱 ${gone.prev ? `0x${gone.prev.toString(16)}` : 'null'} 后继 ${gone.next ? `0x${gone.next.toString(16)}` : 'null'}`, `locate`), scene: buildScene(cfg, pos, 'delete') },
      { line: 1, caption: T(`双向重链：$prev.next\\gets q.next$ 且 $next.prev\\gets q.prev$，$free(q)$`, `relink both sides`), scene: afterScene },
      { line: 2, caption: T(`完成：$L=[${after.join(',')||'∅'}]$`, `done`), scene: afterScene },
    ];
  }
  return gen({ ...cfg, op: 'idle', execTick: 0 });
}
const CODE: Record<Op, any> = {
  idle: [T('$heapBase \\gets ASLR$', '$heapBase$'), T('$p\\gets malloc(nodeSize)$ // prev+data+next', '$p\\gets malloc$'), T('$pending$ // 等待执行', '$pending$')] as never,
  get: [T('$p\\gets head$', '$p\\gets head$'), T('for $k\\gets0$ to $n-1$: // 双向均可', 'for $k\\gets0$ to $n-1$:'), T('  $p\\gets p.next$', '  $p\\gets p.next$'), T('return $p.data$', 'return $p.data$')] as never,
  insert: [T('$q\\gets malloc(nodeSize)$', '$q\\gets malloc$'), T('$q.next\\gets p.next;\\; q.prev\\gets p$', '$q.next\\gets p.next$; $q.prev\\gets p$'), T('$p.next.prev\\gets q;\\; p.next\\gets q$ // 双向重链', '$p.next.prev\\gets q$; $p.next\\gets q$')] as never,
  delete: [T('$q\\gets p.next$', '$q\\gets p.next$'), T('$p.next\\gets q.next;\\; q.next.prev\\gets p$', '$p.next\\gets q.next$; $q.next.prev\\gets p$'), T('$free(q)$', '$free(q)$')] as never,
};
...
export const doublyLinkedListModule: ModuleDef<Scene, Cfg> = {
  ...
  Render({ scene: _scene }) {
    ...
    scene.nodeSize = Number.isFinite(scene.nodeSize) ? scene.nodeSize : 2 * scene.ptrSize + scene.elemSize;
    const empty = scene.nodes.length === 0;
    return (
      <div style={{ display: 'grid', gap: 10 }}>
        <div style={{ border: '1px solid #c7d2fe', borderRadius: 12, overflow: 'hidden', background: '#eef2ff' }}>
          <div style={{ padding: '8px 10px', fontSize: 11, fontWeight: 800, color: '#4338ca', display: 'flex', gap: 8, alignItems: 'center' }}><span>逻辑视图 · 双向链表</span><span style={{ fontWeight: 400, color: '#64748b' }}>head = {scene.head ? `0x${scene.head.toString(16)}` : 'null'} · {scene.nodes.length} 节点 · nodeSize={scene.nodeSize}B</span></div>
                  ...
                  return (
                    <div key={n.idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                      ...
                      <div style={{ display: 'flex', border: `1.5px solid ${isFocus ? '#0f172a' : COLORS[n.idx % COLORS.length]}`, borderRadius: 10, overflow: 'hidden', background: isFocus ? '#0f172a' : '#fff', boxShadow: isFocus ? '0 4px 16px rgba(15,23,42,.25)' : undefined }}>
                        <div style={{ padding: '6px 8px', textAlign: 'center', minWidth: 72, borderRight: '1px dashed #e2e8f0', background: '#faf5ff' }}><div style={{ fontSize: 9, color: isFocus ? '#c4b5fd' : '#8b5cf6' }}>prev</div><div style={{ fontFamily: 'monospace', fontSize: 10, fontWeight: 700, color: isFocus ? '#fff' : '#0f172a' }}>{n.prev ? `0x${n.prev.toString(16)}` : 'null'}</div><div style={{ fontFamily: 'monospace', fontSize: 8, color: '#94a3b8' }}>{n.bytes.slice(0, scene.ptrSize).map(toHex).join(' ')}</div></div>
                        <div style={{ padding: '6px 8px', textAlign: 'center', minWidth: 52, borderRight: '1px dashed #e2e8f0' }}><div style={{ fontSize: 9, color: isFocus ? '#94a3b8' : '#64748b' }}>data</div><div style={{ fontWeight: 800, color: isFocus ? '#fff' : '#0f172a' }}>{n.data}</div><div style={{ fontFamily: 'monospace', fontSize: 9, color: isFocus ? '#cbd5e1' : '#94a3b8' }}>{n.bytes.slice(scene.ptrSize, scene.ptrSize + scene.elemSize).map(toHex).join(' ')}</div></div>
                        <div style={{ padding: '6px 8px', textAlign: 'center', minWidth: 72, background: '#f8fafc' }}><div style={{ fontSize: 9, color: isFocus ? '#94a3b8' : '#64748b' }}>next</div><div style={{ fontFamily: 'monospace', fontSize: 10, fontWeight: 700, color: isFocus ? '#38bdf8' : '#0f172a' }}>{n.next ? `0x${n.next.toString(16)}` : 'null'}</div><div style={{ fontFamily: 'monospace', fontSize: 8, color: '#94a3b8' }}>{n.bytes.slice(scene.ptrSize + scene.elemSize).map(toHex).join(' ')}</div></div>
                      </div>
